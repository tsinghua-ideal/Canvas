...
# baseline/cifar10/optimization/validation_experiments/final/20/score
# canvas_log_dir="${current_dir}/collections/validation_experiments/final/20/ranking"
canvas_log_dir="${current_dir}/collections/baseline/cifar10/optimization"
...
CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES python alpha_partial_order_validation_experiment.py --alpha-weight-decay $alpha_weight_decay --alpha-lr $alpha_lr --model $model --root $root --canvas-log-dir $canvas_log_dir --canvas-rounds $canvas_rounds --epochs $epochs \
 ...
 --opt $opt --weight-decay $weight_decay  --warmup-epochs $warmup_epochs --proxyless --use-multi-epochs-loader --clip-grad $clip_grad  --canvas-tensorboard-log-dir $canvas_tensorboard_log_dir --needs-profiler
