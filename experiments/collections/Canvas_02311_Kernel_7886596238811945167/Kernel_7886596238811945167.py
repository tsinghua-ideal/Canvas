...
class Kernel_7886596238811945167(nn.Module):
    def __init__(self, c: int, h: int, w: int):
        ...
        # Kernels
        # FC: p_3
        self.p_3 = nn.Conv2d(self.c, self.c, 1, padding=0, bias=False)
    ...
    def forward(self, x: torch.Tensor):
        ...
        t_3 = self.p_3(t_1)
        # Sin: p_4
        t_4 = torch.sin(t_1 * 1.57)
        # BMM_0_1: p_5
        t_2_lhs = t_1.view(self.n, self.c, self.h * self.w)
        t_0_rhs = t_0.view(self.n, self.c, self.h * self.w).transpose(1, 2)
        t_5 = torch.bmm(t_2_lhs, t_0_rhs).view(self.n, self.c, self.c) / math.sqrt(self.h * self.w)
        # BMax: p_6
        t_6_lhs = t_3.view(self.n, self.c, self.h, self.w)
        t_6_rhs = t_4.view(self.n, self.c, self.h, self.w)
