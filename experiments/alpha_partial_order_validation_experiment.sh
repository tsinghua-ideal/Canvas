...
canvas_log_dir="${current_dir}/collections/validation_experiments/final/${canvas_number_of_kernels}/multi"
canvas_tensorboard_log_dir="${canvas_log_dir}/tensorboard/model_${model}_${opt}_lr_${lr}_alpha_lr_${alpha_lr}_num_${canvas_number_of_kernels}_machine_${HOSTNAME}_GPU_${CUDA_VISIBLE_DEVICES}_warmup_epoch_${warmup_epochs}"
...
CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES python alpha_partial_order_validation_experiment.py \
 ...
 --canvas-log-dir $canvas_log_dir --canvas-rounds $canvas_rounds --epochs $epochs \
 ...
 --clip-grad $clip_grad  --needs-valid --scale $scale --crop-pct $crop_pct  \
