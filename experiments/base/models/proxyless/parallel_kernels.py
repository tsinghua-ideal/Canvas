...
class ParallelKernels_Test(nn.Module):
    def __init__(self, kernel_cls_list, **kwargs):
        super().__init__()
        assert len(kernel_cls_list) > 1
    ...
    def set_indices(self, active_idx, inactive_idx, active_only):
        ...
        if inactive_idx is not None:
            ...
            for i in (active_idx, inactive_idx):
    ...
    def restore_all(self):
        ...
        if self.inactive_idx is not None:
            assert self.module_list[self.inactive_idx] is not None
            for i in (self.active_idx, self.inactive_idx):
