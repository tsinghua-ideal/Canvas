...
def save(args, kernel_pack_list, best_kernel, train_metrics, eval_metrics, extra):
    if args.canvas_log_dir:
        # Logging info.
        logger = get_logger()
        if best_kernel:
            logger.info(f'Saving kernel {best_kernel.name} into {args.canvas_log_dir} ...')
        else:
        ...
        if 'exception' in extra:
            ...
            dir_name = dir_name + (f'{best_kernel.name}' if best_kernel else f'{time.time_ns()}')
        else:
            ...
            dir_name = f'Canvas_{score_str}_{best_kernel.name}'
