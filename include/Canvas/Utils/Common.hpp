...
static constexpr size_t kSizeTUnlimited = std::numeric_limits<size_t>::max();
...
struct Range {
    ...
    [[nodiscard]] bool IsPoint() const { return min == max; }
...
static auto StaticCast(const PtrType& ptr) {
...
std::string PrettyRatio(const T1& lhs, const T2& rhs) {
    assert(rhs != 0);
    auto ratio = static_cast<double>(lhs) / static_cast<double>(rhs);
